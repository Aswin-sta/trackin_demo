...
import LDRequestCard from "./LDRequestCard";

const LDRequestCardHandler = ({
  status,
}: {
  ...
  const handleApproveClick = async (id: number) => {
    try {
      ...
      const data: requestType[] = [{ id: id, status: "Approved by L&D" }];
      const response = await axiosInstance.put(
        "/nomination/put-nomination-requests",
        data
      );
  ...
  const handleApproveAllClick = async () => {
    try {
      ...
      courses.forEach((course) => {
        requestData.push({ id: course.id, status: "Approved by L&D" });
      });
...
export default LDRequestCardHandler;
